...
Static Function ImpDados(cAlias)
  ...
  While (cAlias)->(!EOF()) 
    if cPedidoAnterio<>alltrim((cAlias)->(C7_NUM))
      nValorTotal:= 0
      Cabecalho()
      CabecPed()
      ...
      CabecItem()
      nPaginas++
      cPedidoAnterio:= alltrim((cAlias)->(C7_NUM))
    endif
    ...
    IncProc()
    (cAlias)->(DbSkip())
    if (cAlias)->(EOF()) .or. cPedidoAnterio<>alltrim((cAlias)->(C7_NUM))
      oPrint:Line(730, 15, 730, 580,,'-6')
      oPrint:Say(750,20,'Pagina '+cValToChar(nPaginas), oFont16,, PRETO)
